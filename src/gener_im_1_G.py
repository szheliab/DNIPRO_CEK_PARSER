...
class ImageRenderer:
    ...
    def _draw_right_header(
        self, draw: ImageDraw.Draw, font: ImageFont.FreeTypeFont
    ) -> None:
        ...
        x0_bg = (
            ...
            - Config.SPACING
            - w_right
            - 2 * Config.RIGHT_TITLE_PADDING
        )
        y0_bg = Config.SPACING
        x1_bg = (
            Config.SPACING * 2 + Config.LEFT_COL_W + 24 * Config.CELL_W - Config.SPACING
        )
        ...
        draw.text((text_x, text_y), right_title, fill=Config.HIGHLIGHT_COLOR, font=font)
    ...
    def _draw_hours_header(self, draw: ImageDraw.Draw, day_keys: list) -> None:
        ...
        table_y0 = (
            Config.SPACING + Config.HEADER_H + Config.HOUR_ROW_H + Config.HEADER_SPACING
        )
        ...
        for h in range(24):
            ...
            x1 = x0 + Config.CELL_W
            draw.rectangle(
                [x0, hour_y0, x1, hour_y1],
                fill=Config.HEADER_BG,
                outline=Config.GRID_COLOR,
            )
    ...
    def _draw_dates_column(self, draw: ImageDraw.Draw, day_keys: list) -> None:
        ...
        draw.rectangle(
            [
                table_x0,
                table_y0 - Config.HOUR_ROW_H,
                table_x0 + Config.LEFT_COL_W,
                table_y0,
            ],
            fill=Config.HEADER_BG,
            outline=Config.GRID_COLOR,
        ...
        draw.text(
            (
                table_x0 + (Config.LEFT_COL_W - w_header) / 2,
        ...
        for r, day_key in enumerate(day_keys):
            ...
            draw.rectangle(
                [table_x0, y0, table_x0 + Config.LEFT_COL_W, y0 + Config.CELL_H],
                fill=Config.TABLE_BG,
                outline=Config.GRID_COLOR,
            )
            ...
            draw.text(
                (table_x0 + (Config.LEFT_COL_W - w) / 2, y0 + (Config.CELL_H - h) / 2),
                date_label,
                fill=Config.TEXT_COLOR,
                font=font_date,
            )

    def _draw_split_cell(
        self,
        draw: ImageDraw.Draw,
        x0: int,
        y0: int,
        ...
        if state == "no":
            ...
            if next_state == "no":
                ...
                right_color = (
                    Config.OUTAGE_COLOR
                    if next_state == "first"
                    else Config.POSSIBLE_COLOR
                )
            elif next_state in ["second", "msecond"]:
                right_color = (
                    Config.AVAILABLE_COLOR
                )  # Перша половина наступної години зі світлом
            else:
            ...
            if prev_state == "no":
                ...
                left_color = (
                    Config.AVAILABLE_COLOR
                )  # Друга половина попередньої години зі світлом
            else:
    ...
    def _draw_grid(self, draw: ImageDraw.Draw, day_keys: list) -> None:
        ...
        for i in range(25):
            ...
            draw.line(
                [(x, table_y0 - Config.HOUR_ROW_H), (x, table_y1)],
        ...
        for r in range(n_rows + 1):
            ...
            draw.line([(table_x0, y), (table_x1, y)], fill=Config.GRID_COLOR)
    ...
    def _draw_legend(self, draw: ImageDraw.Draw, day_keys: list) -> None:
        ...
        for col, text, state in legend_items:
            ...
            draw.rectangle(
                [x_cursor, legend_y, x_cursor + box_size, legend_y + box_size],
                fill=col,
                outline=Config.GRID_COLOR,
            )
            ...
            draw.text(
                (
                    x_cursor + box_size + 4,
                    legend_y + (box_size - (text_bbox[3] - text_bbox[1])) / 2,
                ),
                text,
                fill=Config.TEXT_COLOR,
                font=font_legend,
    ...
    def _draw_footer(self, draw: ImageDraw.Draw) -> None:
        ...
        pub_text = (
            fact.get("update")
            or self.data.get("lastUpdated")
            or datetime.now(ZoneInfo(Config.TIMEZONE)).strftime("%d.%m.%Y")
        )
        ...
        legend_bottom = (
            ...
            + Config.HOUR_ROW_H
            + Config.HEADER_SPACING
            + len(self.processor.get_dates_for_display(self.data)) * Config.CELL_H
            + Config.LEGEND_H
        )

        draw.text(
            (width - w_pub - Config.SPACING, legend_bottom - 20),
            pub_label,
            fill=Config.FOOTER_COLOR,
            font=font_small,
        )
    ...
    def _save_image(self, img: Image.Image) -> None:
        ...
        img_resized = img.resize(
            (img.width * Config.OUTPUT_SCALE, img.height * Config.OUTPUT_SCALE),
